...
import { bookingAccessCode, createAgreementToken, json, requireInvite } from '../_lib/security.js';
...
export default async function handler(request, response) {
  ...
  try {
    ...
    await appendBookingRecord({ type: 'status', bookingId: booking.id, changes: { status: 'reserved', approvedChoice: choiceIndex, amountCents: complimentary ? 0 : (option.amountCents || undefined), paymentPlan: complimentary ? 'complimentary' : booking.paymentPlan, complimentary, reservedAt: selectedAt, selectedByGuestAt: selectedAt }, createdAt: selectedAt });
    ...
    const costText = complimentary ? ' This is a complimentary stay with no rent, taxes, or government lodging fees.' : estimatedTax ? ` The listed cost is $${(option.amountCents / 100).toFixed(2)} before an estimated $${(estimatedTax.estimatedTaxesAndFeesCents / 100).toFixed(2)} in taxes and government lodging fees.` : '';
    ...
    if (guestEmail) emailTasks.push(sendEmail({
      ...
      templateKey:'stay-selected', templateVariables:{ guestName:invite.label, arrival:option.arrival, departure:option.departure, packetUrl, bookingCode },
      text: `Hi ${invite.label},\n\nYou chose ${safeDates}.${costText} We released the other offered dates.\n\nOpen your private reservation details to track payment, sign the rental agreement, and download your paperwork: ${packetUrl}\nGuest Guide reservation code: ${bookingCode}\n\nCancellation policy: cancel at least 24 hours before the 4:00 PM Eastern check-in time for a full refund. Inside 24 hours, the 20% reservation deposit is retained and any remaining amount paid is refunded. We will follow up with any remaining payment or stay details.`,
      html: `<div style="font-family:Arial,sans-serif;color:#332820;line-height:1.6;max-width:600px"><h1 style="color:#183c2d">Your stay choice is reserved</h1><p>Hi ${safeName},</p><p>You chose <strong>${safeDates}</strong>.</p>${complimentary?'<p style="background:#e8f2e9;padding:12px;border-radius:8px"><strong>Our guest:</strong> This is a complimentary stay. No rent, taxes, or government lodging fees are due.</p>':estimatedTax ? `<p>Listed cost: <strong>$${(option.amountCents / 100).toFixed(2)}</strong><br>Does not include an estimated <strong>$${(estimatedTax.estimatedTaxesAndFeesCents / 100).toFixed(2)}</strong> in taxes and government lodging fees.</p>` : ''}<p><a href="${packetUrl}" style="display:inline-block;background:#a45d41;color:#fff;padding:13px 20px;border-radius:999px;text-decoration:none;font-weight:bold">Open reservation details</a></p><p><strong>Guest Guide reservation code:</strong> ${bookingCode}</p><p>Track payment, sign the rental agreement, and download your paperwork there.</p><p style="background:#fff0cc;padding:12px;border-radius:8px"><strong>Cancellation policy:</strong> Cancel at least 24 hours before the 4:00 PM Eastern check-in time for a full refund. Inside 24 hours, the 20% reservation deposit is retained and any remaining amount paid is refunded.</p><p>We released the other offered dates. We will follow up with any remaining payment or stay details.</p></div>`,
    }));
