...
export default async function handler(request, response) {
  ...
  try {
    ...
    for (let booking of stored) {
      ...
      if (booking.paymentPlan==='deposit-balance' && !booking.paymentRequirementMet && approvalAge>=depositReminderAge && !booking.depositReminderSentAt && hour>=9) sent+=await scheduledSend(booking,'deposit-reminder',common,'depositReminderSentAt');
      else if (booking.paymentPlan==='deposit-balance' && !booking.paymentRequirementMet && booking.depositReminderSentAt && Date.now()-Date.parse(booking.depositReminderSentAt)>=20*3600000 && hour>=9) sent+=await scheduledSend(booking,'deposit-grace',common,'depositGraceSentAt');
      if (booking.paymentPlan==='deposit-balance' && !booking.paymentFullyPaid && booking.balanceDueDate && today===shiftDate(booking.balanceDueDate,-1) && hour>=9) sent+=await scheduledSend(booking,'balance-reminder',common,'balanceReminderSentAt');
      if (booking.paymentPlan==='deposit-balance' && !booking.paymentFullyPaid && booking.balanceDueDate && today>booking.balanceDueDate && hour>=9) sent+=await scheduledSend(booking,'balance-grace',common,'balanceGraceSentAt');
    ...
    if(calendar.doorCodeRetirement?.required!==false)codeTasks.unshift({type:'retire',guestName:'Current shared code',label:'Remove the old shared code from every cabin door'});
