...
import { doorCodeEmailDue } from '../_lib/door-code-release.js';
...
export default async function handler(request, response) {
  ...
  try {
    ...
    for (let booking of stored) {
      ...
      if(doorCodeEmailDue(booking)){const firstName=guestFirstName(booking.name);const result=await sendEmail({to:booking.email,toName:firstName,subject:'Your Weeks Creek Haven door code',text:`Hi ${firstName},\n\nYour private cabin door code is ${booking.doorCode}. It is also available now in your Reservation Details and the Check In page of your Guest Guide. Please keep it within your registered group. Check-in begins at 4:00 PM Eastern on ${dates.arrival}.`,html:`<div style="font-family:Arial,sans-serif;color:#332820;line-height:1.6"><h1 style="color:#183c2d">Your cabin door code</h1><p>Hi ${escapeEmailHtml(firstName)},</p><p>Your private cabin door code is:</p><p style="font-size:30px;font-weight:800;letter-spacing:.14em">${booking.doorCode}</p><p>It is also available now in your Reservation Details and the Check In page of your Guest Guide. Please keep it within your registered group.</p><p><strong>Check-in begins at 4:00 PM Eastern on ${dates.arrival}.</strong></p></div>`,idempotencyKey:`${booking.id}-door-code-${booking.doorCodeGeneratedAt || booking.doorCodeInstalledAt}`});if(!result?.skipped){const sentAt=new Date().toISOString();await appendBookingRecord({type:'status',bookingId:booking.id,changes:{doorCodeGuestSentAt:sentAt},createdAt:sentAt});sent++;}}
