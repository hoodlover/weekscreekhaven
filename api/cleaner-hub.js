...
export default async function handler(request,response){
  ...
  try{
    ...
    } else if(action==='owner-cleaning-complete'){
      if(!owner)return json(response,403,{error:'Only the owner can override a cleaning record.'});
      const bookingId=safe(request.body?.bookingId,100); if(!bookingId)return json(response,400,{error:'Choose a cleaning record.'});
      const state=await getCleanerState(),bookings=await getBookingRequests(),known=state.assignments.some(item=>item.bookingId===bookingId)||bookings.some(item=>item.id===bookingId);
      if(!known)return json(response,404,{error:'That cleaning record was not found.'});
      await appendCleanerRecord({type:'assignment',bookingId,createdAt:now,changes:{ownerCompletedAt:request.body?.completed===true?now:''}});
