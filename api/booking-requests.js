    ...
    if (emailConfigured()) {
      ...
      try {
        await sendEmail({
          ...
          text: `Hi ${name},\n\nWe received your preferred date${hasSecondChoice ? ' choices' : ' choice'} for Weeks Creek Haven.${dateChoices[0].quote.friendsAndFamilyDiscount ? ` Welcome! Your ${dateChoices[0].quote.friendsAndFamilyDiscount.label} gives you a discounted rate of ${money(dateChoices[0].amountCents)} for choice 1, saving ${money(dateChoices[0].quote.discountAmountCents)}.` : ` Your current estimate for choice 1 is ${money(dateChoices[0].amountCents)}.`} The standard $200 cleaning cost is included.${lateCheckout ? ' Your estimate includes the $50 noon checkout option.' : ' Standard checkout is 11:00 AM.'} Check-in begins at 4:00 PM. Taxes and any refundable deposit are confirmed before payment. This is a request, not a confirmed reservation.\n\nThank you!`,
          html: `<div style="font-family:Arial,sans-serif;color:#332820;line-height:1.6;max-width:600px"><h1 style="color:#183c2d">We got your request</h1><p>Hi ${safeName},</p><p>We received your preferred date${hasSecondChoice ? ' choices' : ' choice'} for Weeks Creek Haven.</p>${dateChoices[0].quote.friendsAndFamilyDiscount ? `<p style="background:#e8f2e9;padding:14px;border-radius:10px"><strong>Welcome!</strong> Your ${escapeEmailHtml(dateChoices[0].quote.friendsAndFamilyDiscount.label)} gives you a discounted rate of <strong>${money(dateChoices[0].amountCents)}</strong> for choice 1. You save ${money(dateChoices[0].quote.discountAmountCents)}.</p>` : `<p>Current choice 1 estimate: <strong>${money(dateChoices[0].amountCents)}</strong></p>`}<p><span style="color:#74685e">Standard $200 cleaning cost included · ${lateCheckout ? '$50 noon checkout included' : '11:00 AM checkout'} · 4:00 PM check-in · taxes and any refundable deposit confirmed before payment</span></p><p><strong>This is a request, not a confirmed reservation.</strong> We’ll review the calendar and get back to you with availability and next steps.</p><p>Thanks for thinking of the Haven!</p></div>`,
        });
