...
export async function automaticallyApproveBooking(booking) {
  ...
  await sendEmail({to:booking.email,toName:booking.name,templateKey:'booking-approved',templateVariables:{guestName:booking.name,arrival:dates.arrival,departure:dates.departure,total:`$${(amountCents/100).toFixed(2)}`,paymentText:`${paymentText}${deadline}`,packetUrl,checkout:booking.lateCheckout?'noon':'11:00 AM'},subject:'Complete your Weeks Creek Haven booking',text:`Hi ${booking.name},\n\nYour booking checkout for ${dates.arrival} through ${dates.departure} is ready. These dates remain available to other guests until your deposit is received; the first completed deposit secures them. Total: $${(amountCents/100).toFixed(2)}. ${paymentText}${deadline}\n\nSign and manage your booking: ${packetUrl}`,html:`<div style="font-family:Arial,sans-serif;color:#332820;line-height:1.6;max-width:600px"><h1 style="color:#183c2d">Complete your booking</h1><p>Hi ${escapeEmailHtml(booking.name)},</p><p>Your checkout is ready for <strong>${dates.arrival} through ${dates.departure}</strong>. These dates remain open until a deposit is received; the first completed deposit secures them. Your total is <strong>$${(amountCents/100).toFixed(2)}</strong>.</p><p>${escapeEmailHtml(paymentText+deadline)}</p><p><a href="${packetUrl}" style="display:inline-block;background:#183c2d;color:#fff;padding:13px 20px;border-radius:999px;text-decoration:none;font-weight:bold">Open booking packet and sign</a></p></div>`});
