...
function mode() {
  ...
  $('owner-lock-help').textContent = value === 'once' ? 'One successful unlock on one selected door. One-time codes are not automatically reissued. Early deletion of single-use codes is not verified yet.' : value === 'scheduled' ? 'The code works only between these Eastern start and end times.' : 'Permanent codes work until you delete their access.';
...
function render() {
  const query = $('owner-lock-search').value.trim().toLocaleLowerCase();
  const shown = entries.filter(entry => ($('owner-lock-show-deleted').checked || entry.status !== 'revoked') && entry.name.toLocaleLowerCase().includes(query));
  $('owner-lock-list').innerHTML = shown.length ? shown.map(entry => {
    const ended = entry.mode === 'scheduled' && Date.parse(entry.endsAt) < Date.now();
    const label = entry.status === 'installed' ? (ended ? 'Expired' : entry.mode === 'once' ? 'One-time command confirmed' : 'Installed') : ({'revoked':'Access deleted','revoking':'Removal needs attention','updating':'Updating locks','pending':'Pending','unconfirmed':'Not yet confirmed — check the lock','not-active':'No longer active','needs-attention':'Needs attention'}[entry.status] || entry.status);
    const window = entry.mode === 'permanent' ? 'Permanent · until deleted' : entry.mode === 'once' ? 'One successful unlock · one door' : `${date(entry.startsAt)} → ${date(entry.endsAt)}`;
    return `<article class="door-code-box"><strong>${escape(entry.name)}</strong><span class="badge">${escape(label)}</span><div><span class="door-code-value">${escape(entry.code)}</span></div><p class="helper">${escape(window)}</p><p class="helper">${entry.doorIds.map(id=>escape(doors.find(d=>d.id===id)?.name||id)+(entry.doors[id]?`: ${escape(entry.doors[id].status)}`:'')).join(' · ')}</p>${entry.mode==='once'?'<p class="helper">Early deletion of a single-use code is not verified yet. Its record stays visible.</p>':''}${entry.failure?.reason?`<p class="helper">${escape(entry.failure.reason)}</p>`:''}<div class="booking-tools"><button type="button" class="copy" data-copy-owner-lock="${escape(entry.id)}">Copy PIN</button>${entry.mode==='once'?`<button type="button" class="ghost" data-owner-lock-action="check" data-lock-id="${escape(entry.id)}">Check status</button>`:entry.status!=='revoked'?`${!ended&&['pending','needs-attention','updating'].includes(entry.status)?`<button type="button" class="primary" data-owner-lock-action="retry" data-lock-id="${escape(entry.id)}">Retry installation</button>`:''}<button type="button" class="danger" data-owner-lock-action="revoke" data-lock-id="${escape(entry.id)}">${entry.status==='revoking'?'Retry deletion':'Delete access'}</button>`:''}</div></article>`;
  }).join('') : '<div class="empty">No matching access codes. Deleted access is available in history.</div>';
}
async function load() {
  try {
...
async function action(body) {
  ...
  try {
    ...
    $('owner-lock-message').textContent = data.entry.status === 'installed' ? 'Confirmed by KK Home. Check the keypad before sharing a new code.' : data.entry.status === 'revoked' ? 'Access deleted from every assigned door. The record is saved in history.' : 'Saved. Review the code status below.';
...
$('owner-lock-search').addEventListener('input',render);
$('owner-lock-show-deleted').addEventListener('change',render);
$('owner-lock-list').addEventListener('click',async event=>{
  ...
  if(button.dataset.ownerLockAction === 'revoke') {
    const entry=entries.find(item=>item.id===button.dataset.lockId);
    const targets=entry.doorIds.map(id=>doors.find(door=>door.id===id)?.name||id).join(', ');
    if(!window.confirm(`Delete access for ${entry.name} from ${targets}? This removes this PIN from every listed door and keeps its history.`))return;
  }
