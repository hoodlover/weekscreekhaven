...
Owner Lock codes (`admin.html?tab=locks`) supports named six-digit PINs, selected doors, native permanent access (`attribute:0`), and scheduled Eastern windows (`attribute:1`). Single-use codes are restricted to one door and use the app’s dedicated encrypted `/v3/device/insert-temp-pwd` command, with saved `attribute:253` key-list checks. The app uses `/v3/device/get-temporary-key` for its video-call temporary-code display; it must not be treated as saved single-use inventory. Their commands are persisted before sending and never automatically reissued after uncertainty or use. Clear an unused one-time code early in KK Home; generic slot removal is not assumed to revoke the temporary-PIN channel. Owner entries are encrypted, PINs are checked against saved guest/cleaner/owner codes, and operations serialize under a conditional Blob lease. Partial ordinary-code installations preserve per-door references for retry/revocation. No emails are sent by this area.

September 8 single-use verification: the deck insert-temp-pwd command was accepted, but no matching single-use cloud key was observed. Physical first-use and second-use rejection tests are pending; API acceptance alone does not prove installation or consumption.

Owner Lock codes offers name search and Delete access for permanent/scheduled entries. Deletion removes the selected named PIN from every assigned door, retains failed removals for retry, and hides the entry only after all assigned doors confirm removal. Show deleted access reveals the retained history. Single-use early deletion remains unavailable pending verified provider removal.
